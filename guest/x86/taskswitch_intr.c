...
#define ERROR_ADDR	0x5fff0000LLU
...
struct tss_desc desc_cur_backup;
...
void cond_9_desc_tss_base(int violation)
{
	...
	if (violation) {
		...
		desc_target->base_ext = (ERROR_ADDR >> 32) & 0xffffffff;
	}
}
...
void cond_11_desc_cur_base(int violation)
{
	...
	if (violation) {
		...
		desc_main->base_ext = (ERROR_ADDR >> 32) & 0xffffffff;
	}
}
